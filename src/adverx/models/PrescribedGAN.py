    ...
    def train_model(self, dataloader):
        ...
        for epoch in epoch_bar:
            ...
            if g_error_gan.item() < best_loss:
                ...
                torch.save(self.netG.state_dict(), os.path.join(models_dir, 'PrescribedGAN', f"PresGAN_{self.in_machine}.pt"))
                torch.save(self.netD.state_dict(), os.path.join(models_dir, 'PrescribedGAN', f"PresDisc_{self.in_machine}.pt"))
                torch.save(self.log_sigma, os.path.join(models_dir, 'PrescribedGAN', f"PresSigma_{self.in_machine}.pt"))
            ...
            if (epoch+1) % self.sample_and_save_freq == 0 or epoch == 0:
                with torch.no_grad():
                    ...
                    plt.axis('off')
                    #plt.savefig(f"PresGAN_{self.in_machine}_epoch_{epoch}.png")
