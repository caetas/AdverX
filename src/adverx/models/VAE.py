...
def create_checkpoint_dir():
  ...
  if not os.path.exists(os.path.join(models_dir, 'VAE')):
...
class VAE(nn.Module):
    ...
    def ood_score(self, recon_x, x, mu, logvar):
        '''Loss function of the model using SSIM or MSE
        Args:
        recon_x: torch.Tensor, reconstructed input tensor
        x: torch.Tensor, input tensor
        mu: torch.Tensor, mean of the latent space
    ...
    def train_model(self, data_loader, epochs):
        ...
        for epoch in epochs_bar:
            ...
            if acc_loss<best_loss:
                best_loss = acc_loss
                torch.save(self.state_dict(), os.path.join(models_dir,'VAE', f"VAE_{self.in_machine}.pt"))
    ...
    def outlier_detection(self, in_loader, out_loader, in_patches, out_patches, display = True, in_array = None):
        ...
        if in_array is None:
            for (data,_,_) in tqdm(in_loader, desc='In-distribution data', leave = False):
                x = data.to(self.device)
                recon_x, mu, logvar = self(x)
        ...
        for (data,_,info) in tqdm(out_loader, desc='Out-of-distribution data', leave = False):
            x = data.to(self.device)
            recon_x, mu, logvar = self(x)
            kld = self.ood_score(recon_x, x, mu, logvar)
            kld = kld.cpu().numpy()
        ...
        if display:
            print(f"Mean Scores OOD: {np.mean(out_scores):.6f}, AUROC: {auroc:.6f}, FPR95: {fpr95:.6f}")
