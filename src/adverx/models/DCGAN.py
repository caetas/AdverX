...
class Discriminator(nn.Module):
    ...
    def outlier_detection(self, in_loader, out_loader, device, in_array=None, display=True):
        ...
        for (imgs,_,_) in tqdm(out_loader, desc='Out-of-distribution', leave=False):
            imgs = imgs.to(device)
            preds = self.forward(imgs)
            preds = preds.cpu().numpy()
            if len(preds.shape) > 2:
    ...
    def train_model(self, dataloader):
        ...
        for epoch in epochs_bar:
            ...
            if acc_g_loss/len(dataloader.dataset) < best_loss and epoch >= 20:
                ...
                torch.save(self.generator.state_dict(), os.path.join(models_dir, 'DCGAN', f"DCGAN_{self.in_machine}.pt"))
                torch.save(self.discriminator.state_dict(), os.path.join(models_dir, 'DCGAN', f"VanDisc_{self.in_machine}.pt"))
